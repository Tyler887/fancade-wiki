...
- The base price are currently only fixed to 10 , 100 , 1k & 10k - Each menu item script **must** only be executed once per game (if it's left continuosly executing the shop will be filled with repeating set of items until it has too many items in it that will cause the "too many menu items!" error)
